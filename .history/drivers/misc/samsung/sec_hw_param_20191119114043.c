		...
		struct device_attribute *attr, char *buf)
{
	...
	offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset, ",\"TZBC\":\"");
	max_cnt = sizeof(p_rst_exinfo->tz_ex_info.cpu_status);
	max_cnt = max_cnt/sizeof(p_rst_exinfo->tz_ex_info.cpu_status[0]);
	for (cnt = 0; cnt < max_cnt; cnt++) {
		if (p_rst_exinfo->tz_ex_info.cpu_status[cnt] > INVALID_WARM_TERM_ENTRY_EXIT_COUNT)
			offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
					"%s", tz_cpu_status[0]);
		else
			offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
					"%s", tz_cpu_status[p_rst_exinfo->tz_ex_info.cpu_status[cnt]]);
	...
	offset += scnprintf((char*)(buf + offset), SPECIAL_LEN_STR - offset,
			...
			p_rst_exinfo->cpu_stuck_info.cpu[0], p_rst_exinfo->cpu_stuck_info.cpu[1], 
			p_rst_exinfo->cpu_stuck_info.cpu[2], p_rst_exinfo->cpu_stuck_info.cpu[3], 
			p_rst_exinfo->cpu_stuck_info.cpu[4], p_rst_exinfo->cpu_stuck_info.cpu[5], 
			p_rst_exinfo->cpu_stuck_info.cpu[6], p_rst_exinfo->cpu_stuck_info.cpu[7]);
